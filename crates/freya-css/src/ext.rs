use freya_core::{
    ...
    prelude::{ContainerExt, ContainerSizeExt, EffectExt, Label, Paragraph, Rect, StyleExt, Svg, TextStyleExt},
};

use crate::{
    parser::{parse_inline, CssProp},
...
/// - [`Svg`]: layout CSS only (size, padding, margin)
...
impl CssExt for Svg {
    fn css(self, css: &str) -> Self {
        apply_layout(self, &parse_inline(css))
    }

    fn class(self, classes: &str) -> Self {
        classes
            .split_whitespace()
            .fold(self, |el, c| apply_layout(el, &StyleSheet::get(c)))
    }
}

fn apply_layout<T>(element: T, props: &[CssProp]) -> T
where
    T: ContainerExt + ContainerSizeExt + Sized,
{
    props.iter().fold(element, |el, prop| match prop {
        CssProp::Width(s) => el.width(s.clone()),
        CssProp::Height(s) => el.height(s.clone()),
